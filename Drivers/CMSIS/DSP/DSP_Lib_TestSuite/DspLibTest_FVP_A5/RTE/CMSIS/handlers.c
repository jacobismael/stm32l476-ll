...
// this will be 0 when we have emulated the instruction and want to execute the
// next instruction this will be 2 when we have performed some maintenance and
// want to retry the instruction in Thumb (state == 2) this will be 4 when we
// have performed some maintenance and want to retry the instruction in ARM
// (state == 4)
