...
/**
  ...
  <code>numStages=4</code> second order stages.
                   \image html BiquadCascade.gif "8th order filter using a
  cascade of Biquad stages" A 9th order filter would be realized with
  <code>numStages=5</code> second order stages with the coefficients for one of
  the stages configured as a first order filter (<code>b2=0</code> and
  <code>a2=0</code>).
  ...
  register which shifts the result by <code>postShift</code> bits.
                   \image html BiquadPostshift.gif "Fixed-point Biquad with
  shift by postShift bits after accumulator" This essentially scales the filter
  coefficients by <code>2^postShift</code>. For example, to realize the
  coefficients <pre> {1.5, -0.8, 1.2, 1.6, -0.9}
  </pre>
...
void arm_biquad_cascade_df1_f32(const arm_biquad_casd_df1_inst_f32 *S,
  ...
  while (stage > 0U) {
    ...
    /* If the block size is not a multiple of 4, compute any remaining output
     * samples here.
    ...
    while (sample > 0U) {
      ...
      /* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 * y[n-1] + a2 *
       * y[n-2] */
      acc = (b[1] * Xn[2]) + (b[2] * Xn[3]) + (b[3] * Xns) + (a[0] * Yn[0]) +
            (a[1] * Yn[1]);
    ...
    vst1q_f32(pState,
              vcombine_f32(vrev64_f32(vget_high_f32(Xn)), vrev64_f32(Yn)));
    pState += 4;
