...
/**
  ...
  in the following order in the not Neon version. <pre> {b10, b11, b12, a11,
  a12, b20, b21, b22, a21, a22, ...}
  </pre>
  ...
  y, then the array has size: 32*x + 5*y and it must be initialized using the
  function arm_biquad_cascade_df2T_compute_coefs_f32 which is taking the
  ...
  <pre>
                   arm_biquad_cascade_df2T_init_f32(&SNeon, nbCascade,
  neonCoefs, stateNeon);
                   arm_biquad_cascade_df2T_compute_coefs_f32(&SNeon,nbCascade,coefs);
  </pre>

  @par             In this example, neonCoefs is a bigger array of size 8 *
  numStages. coefs is the standard array:
