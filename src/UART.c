...
void USART_Init(USART_TypeDef *USARTx) {
  ...
  USARTx->CR1 &= ~USART_CR1_OVER8; // Set oversample to 16 bits
  USARTx->BRR = (SYS_CLK/baudrate); // Set baud rate
  USARTx->CR1 |=
...
uint8_t USART_Read(USART_TypeDef *USARTx) {
  ...
  while (!(USARTx->ISR &
           USART_ISR_RXNE)); // Wait until RXNE (RX not empty) bit is set
...
void USART_Write(USART_TypeDef *USARTx, uint8_t *buffer, uint32_t nBytes) {
  ...
  for (i = 0; i < nBytes; i++) {
    while (
        !(USARTx->ISR & USART_ISR_TXE)); // wait until TXE (TX empty) bit is set
