...
void I2C_Initialization(void) {
  ...
  // Set system clock for I2C1
  RCC->CCIPR |= RCC_CCIPR_I2C1SEL_0;
  RCC->CCIPR &= ~(RCC_CCIPR_I2C1SEL_1);
  // Reset I2C
  RCC->APB1RSTR1 |= RCC_APB1RSTR1_I2C1RST;
  RCC->APB1RSTR1 &= ~(RCC_APB1RSTR1_I2C1RST);
  // Enable analog noise filter, disable digital
...
//    -- Automatically generate a STOP condition after all bytes have been
//    transmitted
// Direction = 0: Master requests a write transfer
// Direction = 1: Master requests a read transfer
//===============================================================================
int8_t I2C_Start(I2C_TypeDef *I2Cx, uint32_t DevAddress, uint8_t Size,
                 uint8_t Direction) {
  ...
  // timeout error detection, or when PE = 0.
  tmpreg = I2Cx->CR2;

  tmpreg &=
      (uint32_t)~((uint32_t)(I2C_CR2_SADD | I2C_CR2_NBYTES | I2C_CR2_RELOAD |
...
//                           I2C Send Data
//===============================================================================
int8_t I2C_SendData(I2C_TypeDef *I2Cx, uint8_t DeviceAddress, uint8_t *pData,
                    uint8_t Size) {
  int i;
  ...
  for (i = 0; i < Size; i++) {
    // TXE is set by hardware when the I2C_TXDR register is empty. It is cleared
    // when the next data to be sent is written in the I2C_TXDR register. while(
    // (I2Cx->ISR & I2C_ISR_TXE) == 0 );

    // TXIS bit is set by hardware when the I2C_TXDR register is empty and the
...
//                           I2C Receive Data
//===============================================================================
int8_t I2C_ReceiveData(I2C_TypeDef *I2Cx, uint8_t DeviceAddress, uint8_t *pData,
                       uint8_t Size) {
  int i;
  ...
  I2C_Start(I2Cx, DeviceAddress, Size,
            READ_FROM_SLAVE); // 0 = sending data to the slave, 1 = receiving
                              // data from the slave
